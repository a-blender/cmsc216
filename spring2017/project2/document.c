...
int remove_line(Document *doc, int paragraph_number, int line_number) {
   ...
   if (doc != NULL) {
      num_lines = doc->paragraphs[paragraph_number - 1].number_of_lines;      
      if (paragraph_number <= doc->number_of_paragraphs && line_number <= num_lines) {
	 if (line_number < num_lines) {
	    ...
	    for (x = line_number - 1; x < num_lines - 1; x++) {
	       ...
	       line = doc->paragraphs[paragraph_number - 1].lines[x];
	       line2 = doc->paragraphs[paragraph_number - 1].lines[x + 1];
